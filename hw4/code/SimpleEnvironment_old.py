...
class SimpleEnvironment(object):
    ...
    def ConstructActions(self):
        L = 0.5
        r = 0.2
        w_wheel = 0.1  # for now, the magnitude
        ...
        for idx in range(int(self.discrete_env.num_cells[2])):  
            ...
            delta_theta = wc[2] - start_config[2]
            dt = delta_theta*L/2/r/w_wheel
            control = Control(w_wheel, -w_wheel, dt)  # left, right, dt, will always turn right
            footprint = self.GenerateFootprintFromControl(start_config, control, stepsize=0.01)
...
if __name__ == "__main__":
    ...
    s = SimpleEnvironment(robot, np.array([0.2, 0.2, 0.2]))
    s.ConstructActions()
